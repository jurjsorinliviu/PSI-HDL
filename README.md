...
## 🎯 Key Features
  ...
  - **Baseline Transparency**: Reports full baseline comparisons; teacher PINN improves over VTEAM on the memristor dataset (+28.7% MAE), while the compressed Ψ-HDL model trades accuracy for compactness/interpretability and HDL readiness
...
| Memristor Device | 3482                    | 96                        | 97.24%          | 3.65×10⁻⁴ A     |
  ...
  **Comprehensive Validation** (10 Experiments):
  ...
  - ✅ Baseline comparison: Reports accuracy–size–interpretability trade-offs across polynomial, LUT, vanilla NN, and VTEAM; teacher PINN improves over VTEAM on the memristor dataset (+28.7% MAE)
...
## 🧾 Reviewer-Response Artifacts (IEEE Access Revision)

The revision adds additional quantitative artifacts used to address reviewer comments (reduced-precision sensitivity, tanh-approximation error, training curves, and threshold sensitivity sweeps).

```bash
  python Code/reviewer_response_experiments.py all
```

**Outputs**:

  - `Code/output/reviewer_response/bitwidth_sweep/bitwidth_sweep_plot.png` (reduced-precision sensitivity)
  - `Code/output/reviewer_response/tanh_approx/tanh_approx_plot.png` (tanh approximation)
  - `Code/output/reviewer_response/training_curves/memristor_training_loss.png` (training curves)
  - `Code/output/reviewer_response/threshold_sweep/threshold_sweep_plot.png` (threshold sweep)
  - `Code/results/validation_results.json` and `Code/results/pytorch_vs_spice_*.csv` (PyTorch-vs-ngspice numerical equivalence on DC sweeps)
...
```
  ...
  **Results**:
  ...
  - Teacher PINN MAE: 1.092×10⁻⁴ A (VTEAM baseline MAE: 1.531×10⁻⁴ A; +28.7% improvement)
  - Compressed Ψ-HDL model (structured): test MAE 3.645×10⁻⁴ A (see “Experiment 10: Comprehensive Baseline Comparison”)
...
```
  ...
  **Results**:
  ...
  - Teacher PINN achieves **28.7% lower MAE** than the VTEAM baseline on the memristor dataset
  - Training time: 180s (teacher PINN) vs 0.05s (VTEAM)
  - Note: the compressed Ψ-HDL model prioritizes compactness/interpretability and does not necessarily outperform VTEAM in raw MAE (see Experiment 10 for the full trade-off table)
...
```
  ...
  │   ├── reviewer_response_experiments.py # Reviewer-response artifact generator
  ...
  │   ├── run_spice_validation.py  # PyTorch-vs-ngspice equivalence checks
...
### Version 1.1.1 (2026-01-07) — Revision Support

  - Added reviewer-response artifact generator: `Code/reviewer_response_experiments.py` (bit-width sweep, tanh approximation, training curves, threshold sweep)
  - Added PyTorch-vs-ngspice numerical equivalence reports: `Code/results/validation_results.json` and `Code/results/pytorch_vs_spice_*.csv`
  - Updated README baseline statements to clarify teacher PINN vs compressed Ψ-HDL trade-offs and documented new artifact paths
